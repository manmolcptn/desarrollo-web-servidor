...
<body>
    <?php 
        if($_SERVER["REQUEST_METHOD"] == "POST"){
            ...
            if ($resultado -> num_rows == 0) echo "<h2>El usuario no existe</h2>";
            else{
                $info_usuario = $resultado -> fetch_assoc();
                $acceso_concedido = password_verify($password, $info_usuario["password"]);
                
                if(!$acceso_concedido) echo "<h2>Contraseña errónea</h2>";
                else {
